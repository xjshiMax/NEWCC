...
size_t writefunc_data(void *ptr, size_t size, size_t nmemb, struct http_result *result) {
    char filename[50];
    // snprintf(filename, 50, "./speech_call/result.%s", result->has_error ? "txt" : result->format);
    snprintf(filename, 260, "%s.%s",result->fileName, result->has_error ? "txt" : result->format);
    if (result->fp == NULL) {
